...
public class MyOrderAdapter extends RecyclerView.Adapter<MyOrderAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MyOrderAdapter.ViewHolder holder, int position) {
        String paroductID = myOrderItemModelList.get(position).getProductId();
        ...
        holder.setData(resource, title, orderStatus, date, rating, paroductID, position);
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private void setData(String resource, String title, String orderStatus, Date date, int rating, String productID, int position) {
            ...
            if (orderStatus.equals("Cancelled")) {
                ...
                orderIndicator.setImageTintList(ColorStateList.valueOf(Color.parseColor("#00FF0A")));
            }
                                ...
                                FirebaseFirestore.getInstance().collection("USERS").document(FirebaseAuth.getInstance().getUid()).collection("USER_DATA").document("MY_RATINGS")
                                    ...
                                    public void onComplete(@NonNull Task<Void> task) {
                                        if (task.isSuccessful()) {
                                            ...
                                            if (DBquries.myRatedIds.contains(productID)) {
                                                DBquries.myRating.set(DBquries.myRatedIds.indexOf(productID), Long.parseLong(String.valueOf(starPosition + 1)));
                                            } else {
